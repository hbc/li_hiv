...
bed = bed[, c("V1", "V2", "V3", "transcript_id", "feature")]

library(biomaRt)
human = useMart("ensembl", dataset="hsapiens_gene_ensembl")
conversions = getBM(attributes=c("ensembl_transcript_id", "hgnc_symbol"), mart=human)
...
collapsed = conversions %>% group_by(ensembl_transcript_id) %>%
    summarise(conv=collapse(hgnc_symbol))
